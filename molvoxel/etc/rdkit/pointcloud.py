...
from .getter import AtomChannelGetter, BondChannelGetter
...
class PointCloudMaker:
    ...
    def split_channel(self, image: ArrayLike) -> Dict[str, ArrayLike]:
        ...
        return {
            channel_name: channel for channel_name, channel in zip(self.channels, image)
        }
...
class MolPointCloudMaker(PointCloudMaker):
    def __init__(
        ...
        assert channel_type in [
            "features",
            "types",
        ], f"channel_type(input: {channel_type}) should be 'features' or 'types'"
    ...
    def get_channels(
        self, rdmol: Mol, out: Optional[NDArray] = None, **kwargs
    ) -> NDArray:
    ...
    def get_features(
        self, rdmol: Mol, out: Optional[NDArrayFloat32] = None, **kwargs
    ) -> NDArrayFloat32:
        if out is None:
            if self.use_bond:
    ...
    def _get_features(
        self, rdmol: Mol, out: NDArrayFloat32, **kwargs
    ) -> NDArrayFloat32:
        if self.use_bond:
            ...
            atom_features = [
                self.__get_atom_feature(atom, **kwargs) for atom in rdmol.GetAtoms()
            ]
            bond_features = [
                self.__get_bond_feature(bond, **kwargs) for bond in rdmol.GetBonds()
            ]
            if num_atoms > 0:
    ...
    def get_types(
        self, rdmol: Mol, out: Optional[NDArrayInt] = None, **kwargs
    ) -> NDArrayInt:
    ...
    def _get_types(self, rdmol: Mol, out: NDArrayInt, **kwargs) -> NDArrayInt:
        if self.use_bond:
            ...
            num_bonds = rdmol.GetNumBonds()
            atom_types = [
                self.__get_atom_type(atom, **kwargs) for atom in rdmol.GetAtoms()
            ]
            bond_types = [
                self.__get_bond_type(bond, **kwargs) for bond in rdmol.GetBonds()
            ]
            if num_atoms > 0:
...
class _MolElementPointCloudMaker(MolPointCloudMaker):
    def __init__(self, atom_getter, bond_getter, channel_type, start_index):
        ...
        super(_MolElementPointCloudMaker, self).__init__(
            atom_getter, bond_getter, channel_type
        )
...
class MolSystemPointCloudMaker(PointCloudMaker):
    def __init__(self, *args, channel_type: str = "features"):
        ...
        for arg in args:
            if isinstance(arg, MolPointCloudMaker):
                maker = _MolElementPointCloudMaker(
                    arg.atom_getter, arg.bond_getter, channel_type, channel_offset
                )
            else:
                atom_getter, bond_getter = arg
                maker = _MolElementPointCloudMaker(
                    atom_getter, bond_getter, channel_type, channel_offset
                )
    ...
    def get_channels(
        self, rdmol_list: List[Mol], out: Optional[NDArray] = None, **kwargs
    ) -> NDArray:
        if self.use_features:
    ...
    def get_features(
        self, rdmol_list: List[Mol], out: Optional[NDArrayFloat32], **kwargs
    ) -> NDArrayFloat32:
        if out is None:
            num_objects = 0
            for rdmol, maker in zip(rdmol_list, self.maker_list):
    ...
    def _get_features(
        self, rdmol_list: List[Mol], out: NDArrayFloat32, **kwargs
    ) -> NDArrayFloat32:
        object_offset = 0
        for rdmol, maker in zip(rdmol_list, self.maker_list):
            if maker.use_bond:
                ...
                num_objects = rdmol.GetNumAtoms()
            maker._get_features(
                rdmol, out[object_offset : object_offset + num_objects], **kwargs
            )
            object_offset += num_objects
        return out

    def get_types(
        self, rdmol_list: List[Mol], out: Optional[NDArrayInt], **kwargs
    ) -> NDArrayInt:
        assert self.use_features is False
        if out is None:
            num_objects = 0
    ...
    def _get_types(
        self, rdmol_list: List[Mol], out: NDArrayInt, **kwargs
    ) -> NDArrayInt:
        object_offset = 0
        for rdmol, maker in zip(rdmol_list, self.maker_list):
            if maker.use_bond:
