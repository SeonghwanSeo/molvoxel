...
from . import func_features, func_single, func_types
...
class Voxelizer(BaseVoxelizer):
    ...
    def _setup_block(self, blockdim):
        ...
        axis = np.arange(self.dimension, dtype=self.fp) * self.resolution - (self.width / 2.0)
    ...
    def get_empty_grid(self, num_channels: int, batch_size: int | None = None, init_zero: bool = True) -> NDArrayFloat:
        if init_zero:
    ...
    def forward(
        self,
        coords: NDArrayFloat,
        center: NDArrayFloat | None,
        channels: None | NDArrayFloat | NDArrayInt,
    ...
    def _check_args_types(
        self, coords: NDArrayFloat, types: NDArrayInt, radii: float | NDArrayFloat, out_grid: NDArrayFloat | None = None
    ):
        V = coords.shape[0]
        C = np.max(types) + 1
        D = H = W = self.dimension
    ...
    def _check_args_single(
        self, coords: NDArrayFloat, radii: float | NDArrayFloat, out_grid: NDArrayFloat | None = None
    ):
        V = coords.shape[0]
        D = H = W = self.dimension
    ...
    def _get_overlap_blocks(
        self, coords: NDArrayFloat, atom_size: NDArray | float
