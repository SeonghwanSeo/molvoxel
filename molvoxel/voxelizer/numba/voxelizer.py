...
class Voxelizer(BaseVoxelizer):
    ...
    def get_empty_grid(
        self, num_channels: int, batch_size: Optional[int] = None, init_zero: bool = True
    ) -> NDArrayFloat:
        if init_zero:
    ...
    def _check_args_single(
        self, coords: NDArrayFloat, radii: Union[float, NDArrayFloat], out_grid: Optional[NDArrayFloat] = None
    ):
        V = coords.shape[0]
        D = H = W = self.dimension
    ...
    def _get_overlap_blocks(
        self, coords: NDArrayFloat, atom_size: Union[NDArray, float]
