...
from molvoxel.etc.rdkit.getter import AtomFeatureGetter, AtomTypeGetter, BondTypeGetter
...
def main(Voxelizer, RandomTransform, pymol, device):
    ...
    def test(pointcloudmaker, ligand_rdmol, protein_rdmol, channel_radii, atom_radii, save_dir):
        ...
        voxelizer = Voxelizer(
            device=device
        )  # resolution=0.5, dimension=64, atom_scale=1.5, radii_type='scalar', density='gaussian'
        ...
        ligand_coords, protein_coords, center = (
            torch.FloatTensor(ligand_coords),
            torch.FloatTensor(protein_coords),
            torch.FloatTensor(center),
        )
        T = transform.get_transform()
        new_ligand_coords, new_protein_coords = T(ligand_coords, center), T(protein_coords, center)
        ligand_rdmol, protein_rdmol = apply_coord(ligand_rdmol, new_ligand_coords), apply_coord(
            protein_rdmol, new_protein_coords
        )
        image = wrapper.run(ligand_rdmol, protein_rdmol, center, radii=1.5, out_grid=grid)
        if pymol:
            wrapper.visualize(f"{save_dir}/{test_name}.pse", ligand_rdmol, protein_rdmol, image, center)
    ...
    pointcloudmaker = MolSystemPointCloudMaker(
        [atom_getter, bond_getter], [atom_getter, bond_getter], channel_type="types"
    )
    ...
    num_atoms = (
        ligand_rdmol.GetNumAtoms()
        + ligand_rdmol.GetNumBonds()
        + protein_rdmol.GetNumAtoms()
        + protein_rdmol.GetNumBonds()
    )
...
if __name__ == "__main__":
    ...
    from molvoxel.voxelizer.torch import RandomTransform, Voxelizer
