...
def main(voxelizer):
    ...
    pointcloudmaker_types = MolSystemPointCloudMaker(
        [atom_getter, bond_getter], [atom_getter, bond_getter], channel_type="types"
    )
    pointcloudmaker_features = MolSystemPointCloudMaker(
        [atom_getter, bond_getter], [atom_getter, bond_getter], channel_type="features"
    )
    ...
    print("Sanity Check")
    for _ in range(2):
        single_out = run_test(  # noqa
            voxelizer, single_grid, coords, center, None, 1.0, random_translation=0.0, random_rotation=False
        ).tolist()
        type_out = run_test(
            voxelizer, grid, coords, center, types, 1.0, random_translation=0.0, random_rotation=False
        ).tolist()
        feature_out = run_test(
            voxelizer, grid, coords, center, features, 1.0, random_translation=0.0, random_rotation=False
        ).tolist()
