...
def main(Voxelizer, RandomTransform, pymol, device):
    ...
    def test(ligand_rdmol, protein_rdmol, atom_radii, save_dir):
        ...
        if pymol:
            ...
            visualizer.visualize_complex(
                f"{save_dir}/{test_name}.pse",
                ligand_rdmol,
                protein_rdmol,
                {"Atom": ligand_image.squeeze(0)},
                {"Atom": protein_image.squeeze(0)},
                center,
                resolution=voxelizer.resolution,
            )
        ...
        protein_image = voxelizer.forward_single(
            protein_coords, center, radii=protein_atom_radii, out_grid=protein_grid
        )
        assert ligand_image is ligand_grid, "INPLACE FAILE"
        assert protein_image is protein_grid, "INPLACE FAILE"
        if pymol:
